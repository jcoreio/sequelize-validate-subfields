...
describe('flattenValidationErrors', () => {
  it('works', () => {
    ...
    const item0 = new ValidationErrorItem('invalid range', 'JSON', 'range', doc.range)
    try {
      validateSubfields(
        function * validateRange({min, max}) {
          if (min >= max) {
    ...
    const item1 = new ValidationErrorItem('must be a string', 'STRING', 'foo', doc.foo)
    const item2 = new ValidationErrorItem('must be a number', 'DECIMAL', 'bar', doc.bar)
